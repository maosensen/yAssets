...
export const zh: ChangelogRelease[] = [
	{
		...
		title: "启动更安静,也更轻",
		summary:
			"监控文件夹不再汇报每次启动都要做的那遍扫描,也不再重读已经认识的文件。",
		changes: [
			{
				kind: "fixed",
				title: "启动时不再每个监控文件夹弹一条提示",
				text: "以前每次启动,每个监控文件夹都会弹一条「已导入 0 个项目,跳过 N 个重复文件」,而且数字次次一样。这遍扫描本身是必要的——app 关着的时候 watcher 看不到新增的文件——但现在它只在真的导入到东西时才出声。真捡到新文件、或者有失败,照旧会告诉你。",
			},
			{
				kind: "improved",
				title: "启动会跳过已经入库的文件",
				text: "同一遍扫描过去会在每次启动时重新读取并哈希每个监控目录里的每个文件——在三个真实的监控文件夹上,是读完 1.0 GB 才得出「什么都没变」。现在自动扫描把文件自己的大小和修改时间当作证据,对得上就跳过、不读内容,读取量大约减半。内容以另一个源路径存在库里的文件仍会被读取。你自己发起的导入没有变化:它始终检查真实字节。",
			},
		],
	},
	{
		...
		title: "重复分组不再挤坏",
		summary:
			"窗口偏窄时,九个文件的分组一行挤成一团。现在这一行知道该先让出什么。",
		changes: [
			{
				kind: "fixed",
				title: "空间不够时重复条目仍然可读",
				text: "文件较多的分组里,素材名整个消失,文件数和大小折成一行一个词的竖条,删除按钮还顶出了行外、标签被截断在半个词上——而那正是「即将删掉几个文件」的数字。现在这一行按固定顺序让位:先让缩略图,因为精确重复组里每一张本来就是同一张图,然后是名字,按钮永不让。",
			},
		],
	},
	{
		...
		title: "一个文件,一个素材",
		summary:
			"监控指向构建工具输出目录的文件夹,会把同一个文件反复收进来。现在不会了。",
		changes: [
			{
				kind: "fixed",
				title: "监控文件夹只导入一次",
				text: "反复重写输出的工具——构建、渲染、导出——会连续好几秒发出文件系统事件,每一波都会开启一次独立的导入。这些导入相互重叠,而本该拦住重复的内容校验发生在拷贝文件之前、而不是与入库同一步,于是每一次都通过了。一张图片曾因此进库九次。现在校验和入库合成同一步,只有最先到的那次算数。已经进库的副本字节完全相同,用「查找重复」一次就能归组清理。",
			},
			{
				kind: "new",
				title: "复制或打开素材的来源路径",
				text: "属性里的「来源」记录着文件从哪导入,但路径太长,显示时被从中间截断。现在旁边多了两个按钮:复制完整路径,以及在访达中打开原件。如果原件已被移动或删除,就打开它原先所在的文件夹。",
			},
			{
				kind: "fixed",
				title: "「查找重复」不再动书签",
				text: "精确重复只针对字节相同的文件。书签的身份是它的链接,而不是为它抓取的封面图,所以两个共用同一张通用预览图的页面——或者封面恰好和库里某张图相同的书签——不会再被列出来等着删除。",
			},
		],
	},
	{
