...
- **Icons:** two families, both routed through `src/components/icons.ts` — components import by semantic name from there ONLY, never from `~icons/*` or `lucide-react` directly. Solar *Linear* (via `unplugin-icons`, build-time SVG, offline) is the default for content/navigation glyphs; the toolbar/chrome control cluster (chevrons, zoom, sort, filter, palette, search) re-exports from `lucide-react` for its uniform 2px stroke — Solar's control glyphs mix bare and enclosed shapes that read as mismatched weights side by side (see the note in `icons.ts`). `ui/**` primitives also render lucide.
